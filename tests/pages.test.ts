...
describe('rich-content JSON round trip', () => {
  ...
  it('stores and retrieves BlockNote JSON content', () => {
    const blockNoteJson = JSON.stringify({
      ...
      children: [
        ...
        { type: 'paragraph', content: [{ type: 'text', text: 'World', styles: { bold: true } }] }
      ]
    })
  ...
  it('preserves content through update cycle', () => {
    const original = JSON.stringify({ type: 'blockContainer', children: [{ type: 'paragraph', content: [{ type: 'text', text: 'v1', styles: {} }] }] })
    const page = service.createPage(db, { title: 'Update Test', pageType: 'rich', content: original })
    const updatedContent = JSON.stringify({ type: 'blockContainer', children: [{ type: 'paragraph', content: [{ type: 'text', text: 'v2', styles: {} }] }] })
