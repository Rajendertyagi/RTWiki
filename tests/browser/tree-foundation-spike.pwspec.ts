...
test.describe('Wunderbaum tree foundation (spike)', () => {
  ...
  test('renders the page hierarchy with custom type icons', async ({
    page,
    seedOwnedPage
  }) => {
...
test.describe('spike drag-and-drop regions (Wunderbaum host)', () => {
  ...
  test('self-drop posts no move and changes nothing', async ({
    page,
    request,
    seedOwnedPage
  }) => {
  ...
  test('a failed server move rolls back and leaves server truth intact', async ({
    ...
    const domBefore = await page.evaluate(() => document.querySelector('[data-testid="page-tree"]')?.textContent ?? '')
    ...
    await expect
      .poll(
        async () =>
          page.evaluate(() => document.querySelector('[data-testid="page-tree"]')?.textContent ?? ''),
        { timeout: 10_000 }
      )
      .toBe(domBefore)
...
test.describe('spike inline rename (Wunderbaum host)', () => {
  test('F2 + Enter sends exactly one PATCH and updates tree, tab, header, API', async ({
    ...
    await expect
      .poll(
        async () => (await listPages(request)).find((p) => p.id === a.id)?.title,
        { timeout: 10_000 }
      )
      .toBe(renamed)
  ...
  test('virtual HTML/CSS/JS rows offer no rename entry point', async ({
    page,
    seedOwnedPage
  }) => {
...
test.describe('spike session restore', () => {
  test('restores expansion with zero open tabs after reload', async ({
    page,
    seedOwnedPage
  }) => {
