...
function makeTempDir(): string {
  const dir =
    join(tmpdir(), `rtwiki-test-${Date.now()}-${Math.random().toString(36).slice(2)}`)
...
describe("resolveRuntimePaths", () => {
  it("should derive paths from exeDir consistently", () => {
    ...
    expect(paths.databasePath).toBe(
      joinPaths(paths.exeDir, "data", "rtwiki.sqlite"),
    )
    expect(paths.attachmentsDir).toBe(
      joinPaths(paths.exeDir, "data", "attachments"),
    )
